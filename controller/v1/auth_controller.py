...
# Authlib and Flask handle the base URL via ProxyFix automatically
REDIRECT_URI = f'{os.getenv("BACKEND_URL", "http://localhost:5000")}/api/v1/auth/callback/google'

IS_PRODUCTION = os.getenv("FLASK_ENV") == "production"

def set_auth_cookie(response, name, value, expires=None):
    """Helper to set cookies with consistent cross-domain settings for production."""
    response.set_cookie(
        name,
        value,
        path='/',
...
def register():
    ...
    if result.get("success"):
        ...
        if result.get("verification_token"):
            set_auth_cookie(res, 'verif_token', result["verification_token"])
            set_auth_cookie(res, 'is_verified', 'false')
            set_auth_cookie(res, 'teacher_id', str(result["id"]))
...
def login():
    ...
    set_auth_cookie(res, 'is_verified', str(result['is_verified']).lower())
    set_auth_cookie(res, 'teacher_id', str(teacher_id))
...
def logout():
    ...
    # Clear metadata cookies
    res.set_cookie('is_verified', '', expires=0, path='/', samesite='None' if IS_PRODUCTION else 'Lax', secure=IS_PRODUCTION)
    res.set_cookie('teacher_id', '', expires=0, path='/', samesite='None' if IS_PRODUCTION else 'Lax', secure=IS_PRODUCTION)
    res.set_cookie('verif_token', '', expires=0, path='/', samesite='None' if IS_PRODUCTION else 'Lax', secure=IS_PRODUCTION)
